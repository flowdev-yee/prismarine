...
class Hud extends LitElement {
  static get styles () {
    return css`
      ...
      .hand-palm {
        position: absolute;
        background: #b97a57;
        box-shadow: inset -7px 0 #8f563b, inset 7px 0 #d5976f;
      }
      ...
      .hand.swinging {
        animation: hand-swing 180ms ease-out;
      }
      ...
      @keyframes hand-swing {
        ...
        50% {
          transform: translate(-22px, 18px) rotate(-58deg) scaleX(1);
        }
        100% {
          transform: rotate(-24deg) scaleX(1);
        }
  ...
  init (renderer, bot, host) {
    ...
    const hasTouchControls = 'ontouchstart' in window || navigator.maxTouchPoints > 0
    if (document.getElementById('options-screen').forceMobileControls || isMobile() || hasTouchControls) {
      this.showMobileControls(true)
    } else {
      this.showMobileControls(false)
    }

    document.addEventListener('mousedown', (e) => this.playHandAnimation(e))
  }

  playHandAnimation (e) {
    if (e.button !== 0 || this.shadowRoot.querySelector('#chat').inChat) return
    const hand = this.shadowRoot.querySelector('#first-person-hand')
    hand.classList.remove('swinging')
    window.requestAnimationFrame(() => hand.classList.add('swinging'))
  }
  ...
  render () {
    return html`
      ...
      <div id="first-person-hand" class="hand" @animationend=${(e) => e.currentTarget.classList.remove('swinging')}>
